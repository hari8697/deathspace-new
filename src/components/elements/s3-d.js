...
export default (props) => {
  const scrollDistance = [0.2, 0.31]
  // const scrollDistance2 = [0.25, 0.35]
  const progress = useTransform(props.scrollYProgress, scrollDistance, [0, 100])
  ...
  const gatsbyImages = desktopImages.map(({ node: image }) => {
    ...
    return (
      <motion.div
        ...
        variants={props.portImagesVariants}
        style={{}}
        key={title}
      >
  ...
  return (
    <Section className="particle__section section__three" noPadding>
      <AnimatePresence>
        {state && (
          <>
            <motion.div
            ...
            >
              <Title variants={props.childVariants}>Projects</Title>
              <Paragraph variants={props.variants}>
                Okay, enough chit-chat, where's the proof you ask?
                <br />
                Here are some of my selected projects for your viewing pleasure.
              </Paragraph>
